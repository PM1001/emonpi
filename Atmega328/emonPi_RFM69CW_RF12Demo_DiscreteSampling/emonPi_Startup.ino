...
void emonPi_startup()                                                     //emonPi startup proceadure, check for AC waveform and print out debug
{
  ...
  pinMode(11,INPUT_PULLUP);            //enable ATmega328 internal pull-up on D11. Shorted to ground if RFM not present.
  if(digitalRead(11)){                 //check if RFM present 
    RF_STATUS=1;
  }
...
if (analogRead(4) > 0) {CT3 = 1; CT_count++;} else CT3=0;              // check to see if CT is connected to CT1 input, if so enable that channel
if (analogRead(5) > 0) {CT4 = 1; CT_count++;} else CT4=0;              // check to see if CT is connected to CT2 input, if so enable that channel
